...
analysis = 'analysis/height_fitness'
output_directory = paste(base_directory[2],'/',analysis ,sep='')

experiments_type = c(  'height_fitness'
...
for (i in 1:length(measures_names))
{
  ...
  for(a in 1:length(aggregations)){
    ...
    graph = graph +   scale_color_manual(values=experiments_type_colors, labels = experiments_type[1]) # Use experiment name as line label
